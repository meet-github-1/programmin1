...
class utilizecoupon implements Comparator<node>{
    ...
    public static void main(String[] args){
        ...
        PriorityQueue<node> pq=new PriorityQueue<node>(new utilizecoupon());
        pq.add(new node(0+nex[0],ncoup[0]));
        int day=1;
        int num=0;
        while(!pq.isEmpty()){
            while(!pq.isEmpty() && (pq.peek().ex<day || pq.peek().ncp<=0)){
                pq.poll();
            }
            if(!pq.isEmpty()){
                num=num+1;
                pq.peek().ncp=pq.peek().ncp-1;
            }
            if(day<n){
                pq.add(new node(day+nex[day],ncoup[day]));
            }
            day++;
        }
